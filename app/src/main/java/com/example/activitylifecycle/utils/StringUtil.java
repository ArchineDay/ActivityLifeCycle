...
//			}
	...
	public static String byteArrayToStr(byte[] byteArray) {
		return byteArrayToStr(byteArray, byteArray.length);
	}

	//数据转换
	public static String byteArrayToStr(byte[] byteArray, int num) {
		if (byteArray == null){
			return null;
		}
		char[] hexArray = "0123456789ABCDEF".toCharArray();
		char[] hexChars = new char[num * 2];
		for (int j = 0; j < num; j++) {
	...
	//数据转换,转二进制字符串
	public static String byteArrayToBinaryStr(byte[] byteArray, int num) {
		if (byteArray == null){
			return null;
		}
		String bstr = "";
		for (int j = 0; j < num; j++) {
			bstr += String.format("%8s", Integer.toBinaryString(byteArray[j] & 0xFF)).replace(' ', '0');
		}
		return bstr;
	}
	 ...
	 */
	public static  String trimSpecChar(String str) {
		String regEx="[`~!@#$%^&*()+=|{}':;'\\[\\].<>/?~！@#￥%……&*（）——+|{}【】'；：”“’。、？]";
